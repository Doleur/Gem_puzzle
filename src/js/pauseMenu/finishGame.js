...
function autoFinishGame () {
  const presentArr = []
  for (let i = 0; i <= size; i++) {
    const temporaryArr = []
    for (let j = 0; j <= size; j++) {
      temporaryArr.push(Number(document.getElementById(`${i}-${j}`).textContent) === 0 ? '' : Number(document.getElementById(`${i}-${j}`).textContent))
    }
    presentArr.push(temporaryArr)
  }
  const solver = new NPuzzleSolver(presentArr)
  arrAutoFinish = solver.solve()
}

export { arrAutoFinish, autoFinishGame }
